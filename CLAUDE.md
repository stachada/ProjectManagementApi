...
| Aggregate references | Independent aggregate roots reference each other by ID (FK) only — never via an object-graph navigation collection. Owned/child entities (e.g. `ProjectMember` owned by `Project`, `Comment`/`Attachment` owned by `ProjectTask`) are the only things that should appear as a navigation collection on their owning aggregate. | Preserves each aggregate's transaction/consistency boundary and keeps every mutation routed through the owning aggregate's own command handlers. Found violated twice in practice as dead/unused navigation collections — `Project.Boards` (removed; `Board` is an independent aggregate root) and `User.ProjectMemberships` (removed; `ProjectMember` is owned by `Project`, not `User`) — both replaced by querying the owning `DbSet` directly via FK, e.g. `db.Boards.Where(b => b.ProjectId == id)` (see `GetBoardById`, `GetBoardTasks`, `CreateBoardValidator`). |
