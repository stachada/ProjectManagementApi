...
### Rate limiting
- [x] Configure ASP.NET Core built-in rate limiting middleware (done — `services.AddRateLimiter(...)` in `ApiServiceExtensions.AddApiServices`, `app.UseRateLimiter()` in `Program.cs`. **Found already partially in place** at the start of this phase: a hardcoded fixed-window-per-IP global limiter with `RejectionStatusCode = 429` already existed, and `OrdinisApiFactory` (integration test host) already nulled out the global limiter for ordinary tests with a comment anticipating this exact phase — "tests that specifically exercise rate limiting should build their own factory/limiter config." This phase replaced the hardcoded limits with config-driven `RateLimitingOptions` (new `RateLimiting` section in `appsettings.json`: `FixedWindow.PermitLimit`/`WindowSeconds`, `SlidingWindow.PermitLimit`/`WindowSeconds`/`SegmentsPerWindow`) and added the missing sliding-window/`Retry-After`/Problem-Details pieces below)
- [x] Fixed window policy: 100 requests / 60 seconds per IP (unauthenticated) (done — unchanged limits, now config-driven)
- [x] Sliding window policy: 500 requests / 60 seconds per authenticated user (done — **design decision, since Phase 8 (JWT auth) doesn't exist yet**: extracted the partition-selection logic into a pure `RateLimitPartitioner.CreatePartition(HttpContext, RateLimitingOptions)` (`Ordinis.Api/Common/`) that branches on `context.User.Identity?.IsAuthenticated` — `true` → sliding window keyed by the `ClaimTypes.NameIdentifier` claim (falls back to `Identity.Name`); `false` (every request today, since nothing sets `HttpContext.User`) → the existing fixed window keyed by client IP. The sliding-window branch is dead code until Phase 8 wires up JWT auth, at which point it activates with no further changes. Being a pure function (same rationale as `TaskMapper`/`HateoasLink` staying ASP.NET-Core-free) let the branching be unit tested today against a fabricated `ClaimsPrincipal` without needing real auth — see `tests/Ordinis.IntegrationTests/Common/RateLimitPartitionerTests.cs`)
- [x] `429 Too Many Requests` response with `Retry-After` header (done — `AddRateLimiter`'s `OnRejected` callback (`ApiServiceExtensions.cs`) reads `RetryAfter` from `RateLimitLease.TryGetMetadata`, sets the `Retry-After` header in whole seconds, and writes a `429` body via the same `ProblemDetailsFactory.Create(...)` every other error response in this API uses — RFC 9457 shape, `application/problem+json`, correlation ID stamped — rather than `AddRateLimiter`'s default plain-text rejection body. **Scope addition, agreed before implementing:** `GET /health` was exempted via `.DisableRateLimiting()` on its endpoint mapping in `Program.cs` — orchestrator liveness/readiness probes poll frequently and getting `429`'d on a probe is a common real-world footgun, not something the checklist's wording ruled out. Covered end-to-end by `tests/Ordinis.IntegrationTests/Common/RateLimitingTests.cs`, which builds its own low-permit-limit client via `services.PostConfigure<RateLimiterOptions>(...)` — `PostConfigure` runs after every `Configure<RateLimiterOptions>` call regardless of registration order, so it deterministically overrides both the production limiter and `OrdinisApiFactory`'s test-suite-wide null-out)

### Response caching
- [x] `Cache-Control` headers on read-only GET endpoints (e.g. `GET /tasks/{id}` → `max-age=30`) (done — built-in `Microsoft.AspNetCore.Mvc.ResponseCacheAttribute`, pairing with the `services.AddResponseCaching()`/`app.UseResponseCaching()` plumbing that already existed since Phase 7's rate-limiting work. **Scope decision, agreed before implementing:** applied only to the 7 single-resource `GetById` actions (`TasksController`, `TasksV2Controller`, `ProjectsController`, `BoardsController`, `OrganizationsController`, `UsersController`) — the same set that already carries `ConcurrencyToken` → `ETag` from the optimistic-concurrency work above, so a 30s cache window pairs naturally with a resource that already has its own staleness signal. List/collection GETs (`GET /tasks`, `GET /projects/{id}/tasks`, etc.) were deliberately left uncached — no cache-invalidation mechanism exists yet, and a stale cached list is a more visible correctness problem than a stale single resource for 30 seconds. Covered by `tests/Ordinis.IntegrationTests/Common/ResponseCachingTests.cs`)
- [x] `Vary: Accept-Encoding, Authorization` on cached responses (done — same `[ResponseCache(..., VaryByHeader = "Accept-Encoding,Authorization")]` attribute as above. The `Authorization` vary-key has no observable effect yet since Phase 8 (JWT auth) isn't implemented and every request is currently anonymous, but costs nothing to declare now so a shared cache entry can't leak across users once `[Authorize]` lands)
